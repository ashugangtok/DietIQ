...
const processReportData = (data: SheetDataRow[]): Meal[] => {
    if (data.length === 0) return [];
    
    // Group rows by meal time
...
const DietReportCard = React.forwardRef<HTMLDivElement, { siteName: string; reportData: Meal[] }>(({ siteName, reportData }, ref) => {
    return (
        <div className="border rounded-lg p-6 bg-white" ref={ref}>
            <div className="text-center mb-6">
                <h2 className="text-3xl font-bold capitalize text-gray-800">{siteName}</h2>
            ...
            {reportData.map((meal, mealIndex) => (
                <div key={mealIndex} className="mb-8 last:mb-0">
                    <h3 className="text-2xl font-semibold mb-4 text-green-700 bg-green-100 p-2 rounded-md">Meal Time: {meal.time}</h3>
...
export function DietReport({ data }: DietReportProps) {
    ...
    const siteOptions = useMemo(() => [...new Set(data.map(row => row.site_name))].sort(), [data]);

    const siteData = useMemo(() => {
        if (!selectedSite) return [];
        return data.filter(row => row.site_name === selectedSite);
    }, [data, selectedSite]);
    
    const reportData = useMemo(() => processReportData(siteData), [siteData]);
    ...
    const handleDownloadPdf = async () => {
        ...
        if (element) {
            ...
            pdf.save(`${selectedSite}-diet-report.pdf`);
        }
    };
    ...
    return (
        <Card className="shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    Select a site to view its detailed diet report, which can be printed or saved as a PDF.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                 <div className="flex flex-wrap items-center gap-4 mb-6 p-4 bg-muted/50 rounded-lg">
                    <Select value={selectedSite} onValueChange={setSelectedSite}>
                        <SelectTrigger className="w-full sm:w-[280px]">
                            <SelectValue placeholder="Select a site to generate a report" />
                        </SelectTrigger>
                        <SelectContent>
                ...
                {selectedSite ? (
                    <DietReportCard siteName={selectedSite} reportData={reportData} ref={cardRef} />
                ) : (
                    <div className="text-center p-12 border-2 border-dashed rounded-lg">
                        <p className="text-muted-foreground">
                           Please select a site to view the report.
                        </p>
                    </div>
                )}
            </CardContent>
